...
async def list_web_dav_action_log(
    context: Context,
    cursor: Annotated[
        str | None,
        Field(
    ...
    filter: Annotated[
        dict | None,
        Field(
            description="If set, return records where the specified field is equal to the supplied value. Valid fields are `path`, `true_path`, `remote_ip`, `success`, `status`, `action_type`, `username` or `created_at`.",
            default=None,
        ),
    ...
    try:
        ...
        list_obj = files_sdk.web_dav_action_log.list(params, options)
        retval = list(list_obj)
        next_cursor = getattr(list_obj, "cursor", None)
        if not retval:
            return "No webdavactionlogs found."
        ...
        response = f"WebDavActionLog Response:\n{markdown_list}"
        if next_cursor:
...
def register_tools(mcp):
    ...
    async def list_web_dav_action_log_tool(
        ...
        return await list_web_dav_action_log(
            context,
            cursor,
            per_page,
            filter,

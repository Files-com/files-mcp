...
async def list_exavault_api_request_log(
    context: Context,
    cursor: Annotated[
        str | None,
        Field(
    ...
    per_page: Annotated[
        int | None,
        Field(
            description="Number of records to show per page.  (Max: 1000, 1,000 or less is recommended).",
            default=None,
        ),
    ],
    filter: Annotated[
        dict | None,
        Field(
            description="If set, return records where the specified field is equal to the supplied value. Valid fields are `request_ip`, `request_method`, `success` or `created_at`.",
            default=None,
        ),
    ...
    filter_prefix: Annotated[
        dict | None,
        Field(
            description="If set, return records where the specified field is prefixed by the supplied value. Valid fields are `request_ip` and `request_method`.",
            default=None,
        ),
        BeforeValidator(coerce_json),
    ],
    ...
    try:
        ...
        list_obj = files_sdk.exavault_api_request_log.list(params, options)
        retval = list(list_obj)
        next_cursor = getattr(list_obj, "cursor", None)
        if not retval:
            return "No exavaultapirequestlogs found."
        ...
        response = f"ExavaultApiRequestLog Response:\n{markdown_list}"
        if next_cursor:
...
def register_tools(mcp):
    ...
    async def list_exavault_api_request_log_tool(
        ...
        return await list_exavault_api_request_log(
            context,
            cursor,
            per_page,
            filter,

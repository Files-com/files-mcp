...
async def list_public_hosting_request_log(
    context: Context,
    cursor: Annotated[
        str | None,
        Field(
    ...
    filter: Annotated[
        dict | None,
        Field(
            description="If set, return records where the specified field is equal to the supplied value. Valid fields are `path`, `remote_ip`, `success` or `created_at`.",
            default=None,
        ),
    ...
    try:
        ...
        list_obj = files_sdk.public_hosting_request_log.list(params, options)
        retval = list(list_obj)
        next_cursor = getattr(list_obj, "cursor", None)
        if not retval:
            return "No publichostingrequestlogs found."
        ...
        response = f"PublicHostingRequestLog Response:\n{markdown_list}"
        if next_cursor:
...
def register_tools(mcp):
    ...
    async def list_public_hosting_request_log_tool(
        ...
        return await list_public_hosting_request_log(
            context,
            cursor,
            per_page,
            filter,

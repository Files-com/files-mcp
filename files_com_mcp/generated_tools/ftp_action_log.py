...
async def list_ftp_action_log(
    ...
    try:
        ...
        list_obj = files_sdk.ftp_action_log.list(params, options)
        retval = list(list_obj)
        next_cursor = getattr(list_obj, "cursor", None)
        if not retval:
            return "No ftpactionlogs found."
        ...
        response = f"FtpActionLog Response:\n{markdown_list}"
        if next_cursor:
...
def register_tools(mcp):
    ...
    async def list_ftp_action_log_tool(
        ...
        return await list_ftp_action_log(
            context,
            cursor,
            per_page,
            filter,

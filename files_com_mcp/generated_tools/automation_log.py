...
async def list_automation_log(
    ...
    filter_prefix: Annotated[
        dict | None,
        Field(
            description="If set, return records where the specified field is prefixed by the supplied value. Valid fields are `correlation_id`, `error_type`, `node_id`, `node_type`, `output` or `path`.",
            default=None,
        ),
        BeforeValidator(coerce_json),
    ],
    ...
    try:
        ...
        list_obj = files_sdk.automation_log.list(params, options)
        retval = list(list_obj)
        next_cursor = getattr(list_obj, "cursor", None)
        if not retval:
            return "No automationlogs found."
        ...
        response = f"AutomationLog Response:\n{markdown_list}"
        if next_cursor:
...
def register_tools(mcp):
    ...
    async def list_automation_log_tool(
        ...
        return await list_automation_log(
            context,
            cursor,
            per_page,
            filter,

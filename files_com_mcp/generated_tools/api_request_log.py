...
async def list_api_request_log(
    context: Context,
    cursor: Annotated[
        str | None,
        Field(
    ...
    Args:
        ...
        filter: If set, return records where the specified field is equal to the supplied value. Valid fields are `api_key_id`, `interface`, `request_id`, `request_ip`, `request_method`, `success`, `user_id` or `created_at`.
    ...
    try:
        ...
        list_obj = files_sdk.api_request_log.list(params, options)
        retval = list(list_obj)
        next_cursor = getattr(list_obj, "cursor", None)
        if not retval:
            return "No apirequestlogs found."
        ...
        response = f"ApiRequestLog Response:\n{markdown_list}"
        if next_cursor:
